...
const BlogRelatedArticles: FC<IProps> = ({ mainArticleId, tags }) => {
  const router = useRouter();
  const [relatedArticles, setRelatedArticles] = useState<IMiniPost[]>([]);
  ...
  useEffect(() => {
    if (postsData) {
      const filteredRelatedArticles = postsData.items.filter(
        (i) => i.id !== mainArticleId
      );
      setRelatedArticles(filteredRelatedArticles);
    }
  }, [mainArticleId, postsData]);

  return (
    <>
      {relatedArticles.length > 0 ? (
        <div className="flex flex-col justify-center items-start w-full border-b-primary border-b-2 mt-10">
            ...
            <div className="flex justify-center items-center my-20 w-full h-[28rem]">
              ...
              {relatedArticles.length > 0 && (
                <Swiper
                  slidesPerView={'auto'}
                  spaceBetween={10}
                  width={relatedArticles.length === 1 ? 300 : null}
                  centeredSlides={relatedArticles.length === 1}
                  ...
                  breakpoints={{
                    768: {
                      slidesPerView:
                        relatedArticles.length > 1 ? 2 : relatedArticles.length,
                      spaceBetween: 20,
                    },
                    1280: {
                      slidesPerView:
                        relatedArticles.length > 2 ? 3 : relatedArticles.length,
                      spaceBetween: 30,
                    },
                  }}
