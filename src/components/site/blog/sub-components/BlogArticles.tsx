...
const BlogArticles = () => {
  ...
  const fetcher = () =>
    PostAgent.blogsList(qs ? `${qs}&${orderQs()}` : `?${orderQs()}`);

  const { data: postsData, error } = useSWR<
    IPaginated<IMiniPost>,
    AxiosError<INestError>
  >(
    `${PostKeys.GET_BLOGS_LIST}${qs ? `${qs}&${orderQs()}` : `?${orderQs()}`}`,
    fetcher
  );
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row justify-center sm:justify-start items-center w-full mt-20 sm:mt-36">
        ...
        <>
          ...
          <div className="flex justify-center items center w-full mt-20">
            <Pagination
              ...
              baseUrl={`${process.env.NEXT_PUBLIC_APP_BASE_URL}/blog`}
            />
          </div>
        </>
      )}
