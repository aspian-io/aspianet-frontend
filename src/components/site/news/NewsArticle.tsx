...
const NewsArticle: FC<IProps> = ({article}) => {
  ...
  const fetcher = () => PostAgent.newsStatistics(article.slug);

  const { data: statData, error } = useSWR<IPostStat, AxiosError<INestError>>(
    `${PostKeys.GET_NEWS_STAT}/${article.slug}`,
    fetcher
  );
      ...
      <div className="relative w-full h-56 xs:h-72 sm:h-96 lg:h-[36rem] xl:h-[46rem] rounded-xl sm:rounded-3xl overflow-hidden mt-4">
        {article.featuredImage?.key && (
          <Image
            ...
            priority
            alt={article.featuredImage.imageAlt ?? 'News Img'}
          />
        )}
        {!article.featuredImage?.key && (
