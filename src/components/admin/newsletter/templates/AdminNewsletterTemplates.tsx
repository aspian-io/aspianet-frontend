...
const AdminNewsletterTemplates = () => {
  ...
  const fetcher = () =>
    AdminPostAgent.newsletterTemplatesList(session, `${qs}${initialSort()}`);
  ...
  const {
    ...
    `${AdminPostKeys.GET_NEWSLETTER_TEMPLATES_LIST}${qs}${initialSort()}`,
    fetcher
  );
      ...
      <div className="flex justify-center items-center w-full space-x-2 py-1">
        <AuthGuard claims={[ClaimsEnum.ADMIN, ClaimsEnum.POST_EDIT]}>
          <Button
            ...
            onClick={() => {
              router.push(`/admin/newsletter/templates/edit/${id}`);
            }}
          >
            <svg
  ...
  const formatData = useCallback(
    (template: IPostEntity): IDataType => {
      return {
        id: template.id,
    ...
    <>
      ...
      <div className="flex flex-col justify-center items-center pb-4 space-y-4">
        <AdminTable
            ...
            items: [
              {
                value: 'Add',
                onClick: () =>
                  router.push('/admin/newsletter/templates/add-new'),
                claims: [ClaimsEnum.ADMIN, ClaimsEnum.POST_CREATE],
              },
              {
                value: 'Trash',
                onClick: () => router.push('/admin/newsletter/templates/trash'),
                claims: [ClaimsEnum.ADMIN, ClaimsEnum.POST_DELETE],
              },
            ],
          }}
          ...
          pagination={{
            baseUrl: `${process.env.NEXT_PUBLIC_APP_BASE_URL}/admin/newsletter/templates`,
