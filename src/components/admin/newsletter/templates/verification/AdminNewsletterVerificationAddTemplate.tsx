...
import { defaultSubscribeDesign } from './default-subscribe-design';
import { defaultUnsubscribeDesign } from './default-unsubscribe-design';

interface IProps {
  settingKey:
    | SettingsKeyEnum.NEWSLETTER_SUBSCRIBE_TEMPLATE_ID
    | SettingsKeyEnum.NEWSLETTER_UNSUBSCRIBE_TEMPLATE_ID;
}

const AdminNewsletterVerificationAddTemplate: FC<IProps> = ({ settingKey }) => {
  const { data: session } = useSession();
  const router = useRouter();
  const emailEditorRef = useRef<EditorRef | null>(null);

  function getAppropriateDesign() {
    if (settingKey == SettingsKeyEnum.NEWSLETTER_SUBSCRIBE_TEMPLATE_ID)
      return defaultSubscribeDesign;
    if (settingKey == SettingsKeyEnum.NEWSLETTER_UNSUBSCRIBE_TEMPLATE_ID)
      return defaultUnsubscribeDesign;
  }
      ...
      onSubmit={async (_, { setSubmitting }) => {
        emailEditorRef.current?.exportHtml(async (data) => {
          ...
          try {
            ...
            router.push('/admin/newsletter/settings');
          } catch (error) {
            setSubmitting(false);
...
export default AdminNewsletterVerificationAddTemplate;
