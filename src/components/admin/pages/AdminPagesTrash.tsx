...
const AdminPagesTrash = () => {
  ...
  const fetcher = () =>
    AdminPostAgent.softDeletedPagesList(session, page, limit);
  const {
    data: pagesData,
    error,
    mutate,
  } = useSWR<IPaginated<Omit<IPostEntity, 'content'>>, AxiosError<INestError>>(
    `${AdminPostKeys.GET_SOFT_DELETED_PAGES}?page=${page}&limit=${limit}`,
    fetcher
  );
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast.success('The page recovered successfully.', {
  ...
  return (
    <>
      <ConfirmModal
        ...
        onConfirm={async () => {
          try {
            ...
            await AdminPostAgent.emptyPagesTrash(session);
            await mutate();
            toast.success('Pages trash emptied successfully.', {
              className: 'bg-success text-light text-sm',
            });
        ...
        text="Are you sure you want to empty the pages trash and delete all the items permanently?"
      />
      <AuthGuard
          ...
          onConfirm={async () => {
            try {
              ...
              if (itemToDelete) {
                ...
                await mutate();
                toast.success('The page deleted successfully.', {
                  className: 'bg-success text-light text-sm',
                });
              } else {
                toast.error('Something went wrong. Please try again later.', {
      ...
      <div className="flex flex-col justify-center items-center pb-4 space-y-4">
        <AdminTable
          selectable={false}
          emptyTrashButton={pagesData?.items && pagesData.items.length > 0}
          ...
          loading={!pagesData}
          pagination={{
            baseUrl: `${process.env.NEXT_PUBLIC_APP_BASE_URL}/admin/pages/trash`,
...
export default AdminPagesTrash;
