...
const AdminProjectCategories = () => {
  ...
  const fetcher = () =>
    AdminTaxonomyAgent.projectCategoriesList(session, `${qs}${initialSort()}`);
    ...
    <>
      ...
      >
        <ConfirmModal
          ...
          onConfirm={async () => {
            try {
              ...
              if (itemToDelete) {
                ...
                await mutate();
                toast.success('The project category moved to trash.', {
                  className: 'bg-success text-light text-sm',
                });
              } else {
                toast.error('Something went wrong. Please try again later.', {
          ...
          show={removeConfirm}
          onConfirmLoading={removeLoading}
          text="Are you sure you want to delete the project category?"
        />
      </AuthGuard>
      <CategoryDetails
      ...
      <AddCategoryForm
        ...
        taxonomyType={TaxonomyTypeEnum.PROJECT_CATEGORY}
      />
      <EditCategoryForm
        initialValues={
        ...
        <AdminTable
          menu={{
            items: [
              ...
              {
                value: 'Trash',
                onClick: () => router.push('/admin/posts/project-categories/trash'),
                claims: [ClaimsEnum.ADMIN, ClaimsEnum.TAXONOMY_DELETE],
              },
            ],
          }}
          ...
          pagination={{
            baseUrl: `${process.env.NEXT_PUBLIC_APP_BASE_URL}/admin/posts/project-categories`,
