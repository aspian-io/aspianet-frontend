...
const AdminProjectCategoriesTrash = () => {
  ...
  const fetcher = () =>
    AdminTaxonomyAgent.softDeletedProjectCategoriesList(session, page, limit);
  const {
    data: categoriesData,
    error,
    mutate,
  } = useSWR<IPaginated<ITaxonomyEntity>, AxiosError<INestError>>(
    `${AdminTaxonomyKeys.GET_SOFT_DELETED_PROJECT_CATEGORIES}?page=${page}&limit=${limit}`,
    fetcher
  );
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast.success('The project category recovered successfully.', {
    ...
    <>
      <ConfirmModal
        ...
        onConfirm={async () => {
          try {
            ...
            await AdminTaxonomyAgent.emptyProjectCategoriesTrash(session);
            await mutate();
            toast.success('Project categories trash emptied successfully.', {
              className: 'bg-success text-light text-sm',
            });
        ...
        text="Are you sure you want to empty the project categories trash and delete all the items permanently?"
      />
      <AuthGuard
      ...
      >
        <ConfirmModal
          ...
          onConfirm={async () => {
            try {
              ...
              if (itemToDelete) {
                ...
                await mutate();
                toast.success('The project category deleted successfully.', {
                  className: 'bg-success text-light text-sm',
                });
              } else {
                toast.error('Something went wrong. Please try again later.', {
          ...
          show={removeConfirm}
          onConfirmLoading={removeLoading}
          text="Are you sure you want to delete the project category permanently?"
        />
      </AuthGuard>
      <div className="flex flex-col justify-center items-center pb-4 space-y-4">
        <AdminTable
          ...
          pagination={{
            baseUrl: `${process.env.NEXT_PUBLIC_APP_BASE_URL}/admin/posts/project-categories/trash`,
...
export default AdminProjectCategoriesTrash;
