...
const AdminTagsTrash = () => {
  ...
  const fetcher = () =>
    AdminTaxonomyAgent.softDeletedTagsList(session, page, limit);
  const {
    data: tagsData,
    error,
    mutate,
  } = useSWR<IPaginated<ITaxonomyEntity>, AxiosError<INestError>>(
    `${AdminTaxonomyKeys.GET_SOFT_DELETED_TAGS}?page=${page}&limit=${limit}`,
    fetcher
  );
  ...
  const data: IDataType[] = useMemo(
    () => (tagsData ? tagsData.items.map((t) => formatData(t)) : []),
    [formatData, tagsData]
  );
