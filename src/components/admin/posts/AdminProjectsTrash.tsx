...
const AdminProjectsTrash = () => {
  ...
  const fetcher = () =>
    AdminPostAgent.softDeletedProjectsList(session, page, limit);
  const {
    data: projectsData,
    error,
    mutate,
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast.success('The project recovered successfully.', {
  ...
  const formatData = useCallback(
    (project: Omit<IPostEntity, 'content'>): IDataType => {
      return {
        ...
        title: project.title,
        slug: project.slug,
        actions: actionsColumn(project.id),
      };
    },
    [actionsColumn]
  );
    ...
    <>
      <ConfirmModal
        ...
        onConfirm={async () => {
          try {
            ...
            await AdminPostAgent.emptyProjectsTrash(session);
            await mutate();
            toast.success('Projects trash emptied successfully.', {
              className: 'bg-success text-light text-sm',
            });
        ...
        text="Are you sure you want to empty the projects trash and delete all the items permanently?"
      />
      <AuthGuard
          ...
          onConfirm={async () => {
            try {
              ...
              if (itemToDelete) {
                ...
                await mutate();
                toast.success('The project deleted successfully.', {
                  className: 'bg-success text-light text-sm',
                });
              } else {
                toast.error('Something went wrong. Please try again later.', {
      ...
      <div className="flex flex-col justify-center items-center pb-4 space-y-4">
        <AdminTable
          ...
          pagination={{
            baseUrl: `${process.env.NEXT_PUBLIC_APP_BASE_URL}/admin/posts/projects-trash`,
