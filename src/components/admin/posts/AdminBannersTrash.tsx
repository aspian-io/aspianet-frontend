...
const AdminBannersTrash = () => {
  ...
  const fetcher = () =>
    AdminPostAgent.softDeletedBannersList(session, page, limit);
  const {
    data: bannersData,
    error,
    mutate,
  } = useSWR<IPaginated<IPostEntity>, AxiosError<INestError>>(
    `${AdminPostKeys.GET_SOFT_DELETED_BANNERS}?page=${page}&limit=${limit}`,
    fetcher
  );
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast.success('The banner recovered successfully.', {
  ...
  const data: IDataType[] = useMemo(
    () => (bannersData ? bannersData.items.map((p) => formatData(p)) : []),
    [bannersData, formatData]
  );
    ...
    <>
      <ConfirmModal
        ...
        onConfirm={async () => {
          try {
            ...
            await AdminPostAgent.emptyBannersTrash(session);
            await mutate();
            toast.success('Banners trash emptied successfully.', {
              className: 'bg-success text-light text-sm',
            });
      ...
      >
        <ConfirmModal
          ...
          onConfirm={async () => {
            try {
              ...
              if (itemToDelete) {
                ...
                await mutate();
                toast.success('The banner deleted successfully.', {
                  className: 'bg-success text-light text-sm',
                });
              } else {
                toast.error('Something went wrong. Please try again later.', {
          ...
          show={removeConfirm}
          onConfirmLoading={removeLoading}
          text="Are you sure you want to delete the banner permanently?"
        />
      </AuthGuard>
      <div className="flex flex-col justify-center items-center pb-4 space-y-4">
        <AdminTable
          selectable={false}
          emptyTrashButton={bannersData?.items && bannersData.items.length > 0}
          ...
          loading={!bannersData}
          pagination={{
            baseUrl: `${process.env.NEXT_PUBLIC_APP_BASE_URL}/admin/posts/banners-trash`,
...
export default AdminBannersTrash;
