...
import { menuItemsValidationSchema } from './menu-items-validation-schema';
...
const AddMenuItemForm: FC<IProps> = ({
  addMenuItemModalShow,
  parent,
  onClose,
  newMenuItemParentId,
  isActiveMenu,
    ...
    >
      ...
      >
        <Formik
          ...
          onSubmit={async (menuItem, { resetForm }) => {
            try {
              await AdminTaxonomyAgent.create(session, {
                ...menuItem,
                slug: `${slugify(menuItem.term)}-${Date.now()}`.toLowerCase(),
