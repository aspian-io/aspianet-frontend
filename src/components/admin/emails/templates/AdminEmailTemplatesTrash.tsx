...
const AdminEmailTemplatesTrash = () => {
  ...
  const fetcher = () =>
    AdminPostAgent.softDeletedEmailTemplatesList(session, page, limit);
  const {
    data: templatesData,
    error,
    mutate,
  } = useSWR<IPaginated<Omit<IPostEntity, 'content'>>, AxiosError<INestError>>(
    `${AdminPostKeys.GET_SOFT_DELETED_EMAIL_TEMPLATES}?page=${page}&limit=${limit}`,
    fetcher
  );
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toast.success('The template recovered successfully.', {
    ...
    <>
      <ConfirmModal
        ...
        onConfirm={async () => {
          try {
            ...
            await AdminPostAgent.emptyEmailTemplatesTrash(session);
            await mutate();
            toast.success('Templates trash emptied successfully.', {
              className: 'bg-success text-light text-sm',
            });
        ...
        text="Are you sure you want to empty the templates trash and delete all the items permanently?"
      />
      <AuthGuard
      ...
      >
        <ConfirmModal
          ...
          onConfirm={async () => {
            try {
              ...
              if (itemToDelete) {
                ...
                await mutate();
                toast.success('The template deleted successfully.', {
                  className: 'bg-success text-light text-sm',
                });
              } else {
                toast.error('Something went wrong. Please try again later.', {
          ...
          show={removeConfirm}
          onConfirmLoading={removeLoading}
          text="Are you sure you want to delete the template permanently?"
        />
      </AuthGuard>
      <div className="flex flex-col justify-center items-center pb-4 space-y-4">
        <AdminTable
          ...
          pagination={{
            baseUrl: `${process.env.NEXT_PUBLIC_APP_BASE_URL}/admin/email/templates/trash`,
...
export default AdminEmailTemplatesTrash;
